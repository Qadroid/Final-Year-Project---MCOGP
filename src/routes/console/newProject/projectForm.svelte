<script lang="ts">
    ...
    export let data: SuperValidated<Infer<ProjectSchema>>
...
<form method="POST" use:enhance class="flex-col w-96 p-5">
    <div>
        <Form.Field {form} name="name">
            <Form.Control let:attrs>
                <Form.Label for="name">Project Name</Form.Label>
                <Input id="name" placeholder="Enter project name" bind:value={$formData.name}/>
                <Form.FieldErrors />
            </Form.Control>
        </Form.Field>
    </div>
    <div>
        <Form.Field {form} name="description">
            <Form.Control let:attrs>
                ...
                <Input id="description" placeholder="Enter project description" bind:value={$formData.description}/>
                <Form.FieldErrors />
            </Form.Control>
        </Form.Field>
    </div>
    ...
    <div>
        {#if $formData.kubeConfigType === "upload"}
            <Form.Field {form} name="kubeConfigFile">
                <Form.Control let:attrs>
                    <Form.Label for="kubeConfigFile">Kube Config File</Form.Label>
                    <Input id="kubeConfigFile" type="file" bind:value={$formData.kubeConfigFile}/>
                    <Form.FieldErrors />
                </Form.Control>
            </Form.Field>
        {:else if $formData.kubeConfigType === "paste"}
            <Form.Field {form} name="kubeConfigPaste">
                <Form.Control let:attrs>
                    <Form.Label for="kubeConfigPaste">Paste Kube Config</Form.Label>
                    <Textarea id="kubeConfigPaste" placeholder="Paste kube config here" bind:value={$formData.kubeConfigPaste}/>
                    <Form.FieldErrors />
                </Form.Control>
            </Form.Field>
        {/if}
