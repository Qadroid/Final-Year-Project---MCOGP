...
export const load: PageServerLoad = async () => {
    ...
    try {
        const volumesRes = await k8sClient.listNamespacedPersistentVolumeClaim('default');
        volumeListRaw = volumesRes.body.items
        console.log(volumeListRaw)
        volumeList = transformPodData(volumeListRaw)
    } catch (err) {
        console.error(err);
    }
